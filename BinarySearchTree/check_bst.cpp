 ...
 * Description:
    ...
    Output: 1 
    Explanation: 
    The left subtree of root node contains node with key lesser than the root node’s key and 
    the right subtree of root node contains node with key greater than the root node’s key.
...
struct BinarySearchTree
{
    ...
    bool check_bst()
    {
        return check_bst(this->root, NULL, NULL);
    }

    bool check_bst(Node *root, Node *left, Node *right)
    {
        if (root == NULL)
            return true;
        ...
        return check_bst(root->left, left, root) && check_bst(root->right, root, right);
    }
